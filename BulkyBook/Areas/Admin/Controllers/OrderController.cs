    ...
    public class OrderController : Controller
    {
        ...
        public IActionResult CancelOrder(int id)
        {
            ...
            if(orderHeader.PaymentStatus == SD.StatusApproved)
            {
                ...
                Refund refuns = service.Create(options);
                orderHeader.OrderStatus = SD.StatusRefunded;
                orderHeader.PaymentStatus = SD.StatusRefunded;
            }
            else
            {
                orderHeader.OrderStatus = SD.StatusCanceleed;
                orderHeader.PaymentStatus = SD.StatusCanceleed;
            }
            _unitOfWork.Save();
        ...
        public IActionResult GetOrderList(string status)
        {
            ...
            {
                Console.WriteLine("Im In Selecting ORders for Customers");
                orderHeaderList = _unitOfWork.OrderHeader.GetAll(u => u.ApplicationUser.Id == claim.Value, includeProperties: "ApplicationUser");
            }
            foreach(var item in orderHeaderList)
            {
                Console.WriteLine(item.ApplicationUser.Name);
            }
            switch (status)
            {
                case "pending":
                ...
                case "completed":
                    ...
                    break;
                case "rejected":
                    orderHeaderList = orderHeaderList.Where(o => o.OrderStatus == SD.StatusCanceleed
